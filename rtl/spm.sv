...
module spm #(
    ...
    // initial begin
    //     $readmemh("/Volumes/fileserver/Projects_/SoC/bootloader/bootloader.hex", mem);
    // end

    // initial begin
    //     $readmemh("/mnt/fileserver/Projects_/SoC/bootloader/bootloader.hex", mem);
    // end
        ...
        if (!i_rst_n) begin
            ...
            if (i_req_valid && o_req_ready) begin
                if (i_req_wmask != 4'b0) begin
                    if (i_req_wmask[0]) mem[phys_addr[ADDR_BITS+1:2]][ 7:0]   <= i_req_wdata[ 7:0];
                    if (i_req_wmask[1]) mem[phys_addr[ADDR_BITS+1:2]][15:8]   <= i_req_wdata[15:8];
                    if (i_req_wmask[2]) mem[phys_addr[ADDR_BITS+1:2]][23:16]  <= i_req_wdata[23:16];
                    if (i_req_wmask[3]) mem[phys_addr[ADDR_BITS+1:2]][31:24]  <= i_req_wdata[31:24];
                end
...
endmodule   // spm - scratchpad memory
